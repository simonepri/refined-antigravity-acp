...
export function getAgyVersion(): string {
  return (
    process.env.REFINED_AGY_OFFICIAL_ACP_VERSION?.trim() ||
    process.env.REFINED_AGY_VERSION?.trim() ||
    DEFAULT_AGY_VERSION
  );
}

function getArchivePrefix(version: string): string {
  return version.startsWith("1.1.") ? "agy-acp-server-agy_acp_server_" : "agy-acp-server-";
}

export function getBinaryUrls(version: string = getAgyVersion()): Record<string, string> {
  ...
  return {
    ...
    "win32-arm64": `https://dl.google.com/agy-extensions/releases/windows/${prefix}${version}-windows-arm64.zip`,
  };
}

export function getBinaryUrl(
  platformKey: string | null | undefined,
  version: string = getAgyVersion(),
): string | null {
  if (!platformKey) return null;
  return getBinaryUrls(version)[platformKey] ?? null;
}
...
export function getPlatformKey(
  ...
  return key in getBinaryUrls() ? key : null;
}
...
export async function ensureAntigravityBinary(): Promise<string> {
  const url = getBinaryUrl(getPlatformKey());
  if (!url) {
    throw new Error(
...
export function agyCommand(
  platform: NodeJS.Platform = process.platform,
  override = process.env.REFINED_AGY_ACP_BIN,
